...
// Scalar triple product (3D only): a · (b × c)
template<typename T>
inline T scalarTripleProduct(const Vec<T, 3>& a, const Vec<T, 3>& b, const Vec<T, 3>& c) {
    return dot(a, cross(b, c));
}

// Vector triple product (3D only): a × (b × c)
template<typename T>
inline Vec<T, 3> vectorTripleProduct(const Vec<T, 3>& a, const Vec<T, 3>& b, const Vec<T, 3>& c) {
...
template<typename T, size_t N>
inline Vec<T, N> reject(const Vec<T, N>& a, const Vec<T, N>& b) {
...
template<typename T>
inline Vec<T, 2> perpendicular(const Vec<T, 2>& v) {
    return Vec<T, 2>(-v.data[1], v.data[0]);
}
...
template<typename T, size_t N>
inline bool isParallel(const Vec<T, N>& a, const Vec<T, N>& b) {
    Vec<T, N> normalizedA = normalize(a);
    Vec<T, N> normalizedB = normalize(b);
    T dotProduct = std::abs(dot(normalizedA, normalizedB));
    return std::abs(dotProduct - T(1)) < Epsilon;
}

// Check if two vectors are perpendicular
template<typename T, size_t N>
inline bool isPerpendicular(const Vec<T, N>& a, const Vec<T, N>& b) {
