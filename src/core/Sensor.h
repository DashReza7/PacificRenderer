...
public:
    ...
    Ray sample_ray(uint32_t row, uint32_t col, const Vec2f &sample2) {
        // Compute sensor space coordinates
        Float aspect_ratio = static_cast<Float>(film.width) / static_cast<Float>(film.height);
        Float px = (static_cast<Float>(col) + sample2.x) / static_cast<Float>(film.width);
        Float py = (static_cast<Float>(row) + sample2.y) / static_cast<Float>(film.height);
