...
private:
    ...
    void createLightPath(const Scene *scene, Sampler *sampler,
        ...
        Vec3f beta = lightLe * std::abs(glm::dot(light_normal, light_dirn)) / (light_pdf_posn * light_pdf_dirn);
        prefix_iscs.push_back(isc);
        prefix_betas.push_back(beta);
...
public:
    ...
    Vec3f sample_radiance(const Scene *scene, Sampler *sampler, const Ray &sensor_ray, int row, int col) const override {
        ...
        int cam_max_length = 2;  // max number of segments
        ...
        // ---------------- Sample a  light path ----------------
        int light_max_length = 2;  // max number of segments
        ...
        int light_vertex_idx = uniformDiscrete(sampler->get_1D(), -1, n_light_vertices - 1);
        Intersection cam_isc = cam_prefix_iscs.at(cam_vertex_idx);
        // handle light_vertex_idx of -1 (no light path vertex)
        if (light_vertex_idx == -1) 
            return cam_isc.shape->emitter ? cam_prefix_betas.at(cam_vertex_idx) * cam_isc.shape->emitter->eval(cam_isc)
                                          : Vec3f{0};
        ...
        if (light_vertex_idx == 0)
            light_bsdfval = Vec3f{std::abs(glm::dot(dirn, light_isc.normal))};
        else
