...
Vec3f PathTracerIntegrator::sample_radiance(const Scene *scene, Sampler *sampler, const Ray &ray, int row, int col) const {
    ...
    if (!hide_emitters) {
        ...
        if (!is_hit) {
            ...
            // FIXME: BUG for hdr envmap maybe?
            Vec3f lightLi = scene->env_map->eval(curr_isc);
            if (!check_valid(lightLi)) throw std::runtime_error("EnvMap invalid");
            return lightLi;
        }
        // area light
        if (curr_isc.shape->emitter && glm::dot(curr_isc.normal, curr_isc.dirn) > 0.0)
            radiance += curr_isc.shape->emitter->eval(curr_isc);
    } else if (is_hit && curr_isc.shape->emitter) {
        return Vec3f{0};
    }
    ...
    for (int depth = 1; depth < max_depth || max_depth == -1; depth++) {
        ...
        if (!curr_isc.shape->bsdf->has_flag(BSDFFlags::Delta)) {
            ...
            if (emitter_sample.is_visible) {
                ...
                if (!check_valid(bsdf_value))
                    throw std::runtime_error("invalid BSDF value in Emitter sampling.");
        ...
        if (!check_valid(bsdf_value) || !check_valid(bsdf_sample.pdf))
            throw std::runtime_error("invalid BSDF smaple.");
        ...
        if (!check_valid(throughput))  throw std::runtime_error("invalid throughput.");

        // RayTrace
        Vec3f wo_world = localToWorld(bsdf_sample.wo, curr_isc.normal);
        curr_ray = Ray{rayOffset(curr_isc, wo_world), wo_world, Epsilon, 1e4};
        is_hit = scene->ray_intersect(curr_ray, curr_isc);
        // accumulate the hit object if emitter
        Vec3f lightLi{0.0};
        if (is_hit) {
